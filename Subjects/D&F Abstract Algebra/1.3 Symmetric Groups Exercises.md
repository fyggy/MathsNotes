...
$$
b) We see that, given an element ${} a_{k}\in \sigma {}$, where ${} \sigma {}$ is some ${} n$-cycle, then ${} \sigma^{i}(a_{k})=a_{k} {}$ iff ${} \sigma^{i}=i {}$. 
...
13. skip
14. 
First we prove a lemma on commuting of cycles. Let ${} \sigma,\, \tau \in  S_{w} {}$ be commuting cycles. Then they are either disjoint or equal.
...
Case 2: ${} \sigma,\, \tau {}$ are not disjoint. . Therefore there is some element in ${} a_{k}\in \sigma {}$ that is also in $\tau {}$. Therefore, we must have ${} a_{n}\in \tau {}$ such that ${} \tau(a_{n-1})=a_{k} {}$, so ${} \sigma(\tau(a_{n-1}))=a_{k+1}=\tau(\sigma(a_{n-1}))=\tau(a_{n}) {}$. Now note that ${} a_{k+i+1}=\sigma^{i}(a_{k+1})=\sigma^{i}(\tau(a_{n}))=\tau(\sigma^{i}(a_{n}))=\tau(a_{n+i}) {}$
