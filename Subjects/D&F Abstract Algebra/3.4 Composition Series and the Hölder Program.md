...
$$
are both composition series. Assume by induction that for ${} n=\min(s,\, r) {}$, all groups with a composition series length of $k<n {}$, the Jordan-Holder Theorem holds.

Suppose that ${} n = 1 {}$. Then ${} G$ is simple, so it holds. 

Now suppose that ${} n{} >1$. Then there exists some non-trivial normal subgroup ${} N_{r-1} {}$ with ${} G / N_{r-1} {}$ simple. Then there exists a unique composition sequence for $N$, and for ${} G / N_{r-1} {}$
