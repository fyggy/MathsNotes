...
Now since $G$ is solvable, then there exists a normal subgroup $N\trianglelefteq G$. Let ${} x \in N {}$. Then ${} \langle x \rangle \trianglelefteq G {}$. Then ${} \bar{G}=G /\langle x \rangle  {}$. Both ${} \langle x \rangle {}$ and $H$ both have order less than $G {}$, and are both solvable so there exist
$$
1 \trianglelefteq X_{1} \trianglelefteq \dots \trianglelefteq X_{a} =\langle x \rangle 
$$
with ${} X_{k+1} / X_{k}  {}$ cyclic, and
$$
\bar{1}=\langle x \rangle \trianglelefteq \bar{G_{1}}\trianglelefteq_{}\dots\trianglelefteq \bar{G_{b}}=\bar{G}
$$
With $$
