...
(iii $\Rightarrow {}$iv)
Let $G {}$ be a finite group of order $n {}$ whose composition factors are all of prime order. Then assume by induction that for all groups of order ${} k< n {}$, then there exists a chain ${} 1=N_{0}\trianglelefteq N_{1}\trianglelefteq\dots\trianglelefteq N_{a}=G {}$ with ${} N_{k+1} /N_{k} {}$ cyclic and $N_{k}\trianglelefteq G {}$.

Now if $G {}$ is simple, then the composition factors of $G {}$ are just ${} G {}$, so $G {}$ is of prime order. Therefore, ${} G\cong \mathbb{Z}_{p} {}$, for some prime $p {}$. Therefore, ${} G\cong G /1 {}$ is cyclic, and $1\trianglelefteq G {}$.
