...
\begin{align}
 ...
 \end{align}
$$
Now let $X \in H_{i+1 \,j} {}$. Let ${} \theta:H_{i+1 \,j} \to{} H_{i\, j+1} {}$, with ${} \theta(X)=\varphi_{i\,j}\left[ \psi_{ij} ^{-1} \left[ X \right]  \right]  {}$. Since these are all well defined operations, then ${} \theta {}$ is well defined. Now we show that the domain of $\theta {}$ is actually ${} H_{i\,j+1} {}$. We have ${} X=e+\psi_{i\,j} \left[ \ker \varphi_{i\,j} \right] {}$ for some ${} e \in \im \psi_{i\,j} \cap \ker \varphi_{i+1 \,j} {}$. Now there exists some ${} b \in E_{ij} {}$ with ${} \psi_{ij}b=e {}$, and
$$
\begin{align}
...
$$
so ${} f \in \ker \psi_{i\,j+1} {}$, so ${} f \in \im \varphi_{ij}\cap \ker \psi_{i\,j+1} {}$. Therefore, ${} \theta(X) \in H_{i\,j+1} {}$. Since $\theta$ is well defined, then this holds for every choice of ${} e {}$ and ${} b {}$.
...
$$
so ${} \varphi_{i\,j} b_{1} - \varphi_{i\,j} b_{2} \in \varphi_{i\,j}[\ker  \psi_{i\,j}] {}$, so ${} b_{1}-b_{2} \in \ker  \psi_{i\,j} {}$, and 
$$
e_{1}-e_{2}=\psi_{i\,j}(b_{1})-\psi_{i\,j}(b_{2})=\psi_{i\,j}(b_{1}-b_{2})=0
$$
so ${} e_{1}=e_{2} {}$.