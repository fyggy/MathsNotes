...
\begin{align}
 ...
 \end{align}
$$
Now let $X \in H_{i+1 \,j} {}$. We have $X=e+\psi_{i\,j} \left[ \ker \varphi_{i\,j} \right] {}$ for some ${} e \in \im \psi_{i\,j} \cap \ker \varphi_{i+1 \,j} {}$. Now there exists some ${} b \in E_{ij} {}$ with ${} \psi_{ij}b=e {}$, and
$$
\begin{align}
 \varphi_{i\,j} \left[ \psi_{i\,j} ^{-1} [e+\psi_{i\,j}[\ker \varphi_{i\,j}]] \right]    & = \varphi_{i\,j} \left[ \psi_{i\,j} ^{-1} [\psi_{i\,j}[b+\ker \varphi_{i\,j}]] \right]    \\
  & =\varphi_{i\,j} \left[ b+\ker \varphi_{i\,j} +\ker \psi_{i\,j}\right]     \\
 & =\varphi_{i\,j} b+\varphi_{i \,j}[\ker \psi_{i\,j}]
 \end{align}
$$
Now let ${} f=\varphi_{i\,j}b {}$. Since ${} \psi_{i\,j+1} \circ \varphi_{ij}=\varphi_{i+1 \,j} \circ \psi_{ij} {}$ and ${} e \in \ker \varphi_{i+1\,j} {}$, then
$$
\psi_{i \,j+1}(f)=\psi_{i\,j+1} \varphi_{ij} b = \varphi_{i+1 \,j} \psi_{ij} b=\varphi_{i+1 \,j} e=0
$$
so ${} f \in \ker \psi_{i\,j+1} {}$, so ${} f \in \im \varphi_{ij}\cap \ker \psi_{i\,j+1} {}$. Therefore, ${} \varphi_{i\,j} \left[ \psi_{i\,j} ^{-1} [X] \right]  \in H_{i\,j+1} {}$. Let ${} \theta:H_{i+1 \,j} \to{} H_{i\, j+1} {}$, with ${} \theta(X)=\varphi_{i\,j}\left[ \psi_{ij} ^{-1} \left[ X \right]  \right]  {}$. Since these are all well defined operations, then ${} \theta {}$ is well defined, and this holds for every choice of ${} e {}$ and ${} b {}$.

Now let ${} X,\, Y \in H_{i+1\,j} {}$, ${} \alpha,\, \beta \in \Gamma {}$. Then since ${} \varphi_{i\,j} {}$ and ${} \psi_{i\,j} {}$ are both linear, then
$$
\begin{align}
 \theta(\alpha X+\beta Y)  & =\varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[\alpha X+\beta Y] \right]  \\
 & =\varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[\alpha X] \right]+\varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[\beta Y] \right] \\
 & = \alpha\varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[X] \right]+\beta \varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[Y] \right] \\

 \end{align}
$$
So ${} \theta$ is linear.

Now let ${} X,\, Y \in H_{i+1\,j} {}$ with $\theta(X)=\theta(Y) {}$. Then we have ${} e_{1},\, e_{2} \in \im \psi_{i\,j} \cap \ker \varphi_{i+1 \,j} {} {}$ with 
$$
\begin{align}
X & =e_{1} +\psi_{i\,j}[\ker \varphi_{i\,j}] \\
Y & =e_{2} +\psi_{i\,j}[\ker \varphi_{i\,j}]
\end{align}
$$
and ${} b_{1},\, b_{2} \in E_{ij} {}$ with ${} \psi_{i\,j} b_{1} = e_{1} {}$ and ${} \psi_{i\,j} b_{2} = e_{2} {}$. Now
$$
\theta(X)=\varphi_{i\,j} b_{1} +\varphi_{i\,j}[\ker  \psi_{i\,j}]= \varphi_{i\,j} b_{2} +\varphi_{i\,j}[\ker  \psi_{i\,j}]=\theta(Y)
$$
so ${} \varphi_{i\,j} b_{1} - \varphi_{i\,j} b_{2} \in \varphi_{i\,j}[\ker  \psi_{i\,j}] {}$, so ${} b_{1}-b_{2} \in \ker  \psi_{i\,j} {} + \ker  \varphi_{i\,j} {}$. Now ${} \psi_{i\,j}(b_{1}-b_{2}) \in \psi_{i\,j} \left[ \ker  \varphi_{i\,j} \right]  {}$, so ${} X=Y {}$. Therefore, $\theta$ is injective.

Now let ${} Z \in H_{i\,j+1} {}$. Then there exists some ${} f \in \im  \varphi_{ i\,j} \cap \ker  \psi_{i\,j+1} {}$ with ${} Z=f+\varphi_{i\,j} [\ker \psi_{i\,j}] {}$. Now we have some ${} b \in E_{i\,j} {}$ with ${} \varphi_{ij} b = f {}$. Let ${} e = \psi_{ij} {}$. Note that ${} \varphi_{i+1 \,j} e = \varphi_{i+1\,j} \psi_{ij} b = \psi_{i\,j+1} \varphi_{ij} b = \psi_{i\,j+1}f=0 {}$, so ${} e \in \ker \varphi_{i+1\,j} {}$. Therefore, ${} X=e+\psi_{i\,j}[\ker \varphi_{i\,j}] \in H_{i+1\,j} {}$. Now we have some ${} a \in E_{ij} {}$ with $$
$$
 \begin{align}
 \theta(X) & =\varphi_{i\,j} \left[ \psi_{i\,j} ^{-1}[e+\psi_{i\,j}[\ker \varphi_{i\,j}]] \right]   \\
 &  =
 \end{align}
$$