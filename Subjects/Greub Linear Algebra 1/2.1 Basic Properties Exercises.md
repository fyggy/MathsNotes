...
$$
ii)
Now suppose $\varphi_{2}$ is surjective, and $\varphi_{5}$ is injective. Then let ${} m \in \beta_{3} ^{-1}(\im \varphi_{4}) {}$. Then ${} \beta_{3}(m)\in \im \varphi_{4} {}$, so there exists some ${} e \in E_{4} {}$ with ${} \beta_{3}(m)=\varphi_{4}(e) {}$