...
3. 
Let ${} K \subseteq E \subseteq F {}$ be fields, and suppose that $E$ is algebraic over $K$, and $F$ is algebraic over $E$. Now if ${} x \in F {}$, then there exists a non-zero monic polynomial ${} \Irr(x:E) \in E[X] {}$ such that ${} f(x)=0 {}$. Now if ${} (a_{i})_{i \in I} {}$ are the coefficients of $f {}$, then ${} f \in K((a_{i})_{i \in I})[X] {}$