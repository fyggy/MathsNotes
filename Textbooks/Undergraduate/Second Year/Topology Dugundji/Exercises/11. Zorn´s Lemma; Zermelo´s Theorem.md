...
Now let ${} \mathscr{M}= \bigcap \{ \mathscr{A} \in \mathscr{P}(\mathscr{P}(X))\mid \mathscr{A} \text{ is a }\varphi \text{-spire} \} {}$. Note that $\mathscr{M}$ is minimal, that is, if ${} \mathscr{A} \subseteq \mathscr{M} {}$ is a $\varphi$-spire, then $\mathscr{A}=\mathscr{M} {}$. Now we wish to show that $\mathscr{M} {}$ is a increasing chain ordered by inclusion. 

Call ${} M \subseteq  \mathscr{M} {}$ *medial* whenever, for all ${} A \subseteq  \mathscr{M} {}$, ${} A \subseteq  M \vee M \subseteq A {}$, and ${} x \in M {}$ and ${} y \in A {}$ First, we establish:
...
3. Let ${} A \in \mathscr{G}_{M} {}$. Then either
	1. ${} M \cup \{ \varphi(M) \} \subseteq A {}$. Then, clearly, ${} M \cup \{ \varphi(M) \} \subseteq A \cup \{ \varphi(A) \} {}$, and ${} A \cup \{ \varphi(A) \} \in \mathscr{G}_{M} {}$
