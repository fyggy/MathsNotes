...
$$
A< B \iff \forall x \in B: \exists y \in A:y \prec x
$$
First note that $< {}$ is clearly a preorder. Now let $\mathscr{A} {}$ be a well ordered subset of ${} \mathscr{F}$, and let ${} Z=\bigcup \mathscr{A} {}$. $Z$ is clearly totally ordered. Now let ${} S \subseteq Z {}$, and let $A$ be the first element of ${} \{ B \in \mathscr{A} \mid B \cap  S \neq  \varnothing  \} {}$, and let $m$ be the first element of ${} S \cap  A {}$. Now suppose that ${} x \in S {}$. Then ${} x \in B {}$ for some ${} B \in \mathscr{A} {}$, and so $A <B {}$. Therefore, there exists some ${} y \in A {}$ such that ${} y\prec x {}$, and $m\prec y\prec x$, so $m\prec x$, and so ${} Z \in \mathscr{F} {}$. 
