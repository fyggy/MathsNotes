...
5. 
Let $X$ be a totally ordered set, and let $\mathscr{F}$ be the set of well ordered subsets of $X$. Define an order $<$ in $\mathscr{F}$ by, if ${} A,\, B \in \mathscr{F} {}$, 
$$
A< B \iff \forall x \in A: \exists y \in B:x \prec y
$$
First note that $< {}$ is clearly a preorder. Now let $\mathscr{A} {}$ be a well ordered subset of ${} \mathscr{F}$, and let ${} Z=\bigcup \mathscr{A} {}$. $Z {}$ is clearly totally ordered, as $X {}$ is totally ordered. Now let ${} S \subseteq Z {}$, and let $A$ be the first element of ${} \{ B \in \mathscr{A} \mid B \cap  S \neq  \varnothing  \} {}$, and let $m {}$ be the first element of ${} S \cap  A {}$. Now suppose that ${} x \in S {}$. Then ${} x \in B {}$ for some ${} B \in \mathscr{A} {}$, and so $A <B {}$. Therefore, there exists some ${} y \in A {}$ such that ${} y\prec x {}$, and $m\prec y\prec x$, so $m\prec x$, and so ${} Z \in \mathscr{F} {}$. 

Now let ${} A \in \mathscr{A} {}$, and ${} x \in A {}$. Then ${} x \in Z {}$, and since ${} x \prec x {}$, there exists some ${} y \in Z {}$ such that $x\prec y$. Furthermore, let ${} x \in Z {}$. Suppose that ${} x \precneqq y {}$ for all ${} y \in A {}$. 
