...
c)
Suppose the axiom of choice. Let $X$ be partially ordered, and let $\mathscr{F}$ be the set of chains on $X$. Then $\mathscr{F}$ is partially ordered by inclusion. Let ${} \mathscr{A} \subseteq \mathscr{F} {}$ be a chain in $\mathscr{F}$. Then if ${} \mathcal{A},\, \mathcal{B} \in \bigcup \mathscr{A} {}$ are chains in ${} \mathscr{A}$, then $$