...
Let $\prec {}$ be a well ordering of ${} E {}$ (not necessarily respecting the pre-existing order). Define ${} S:I(E)\to{}I(E) {}$ by, if ${} S_{1} \subseteq Q {}$ is an ideal, then if ${} S_{1} \subset Q {}$, then there exists some ideal ${} S_{1} \subset S_{2} \subseteq Q {}$, and set
$$
S(S_{1})=\begin{cases}
S_{2},  & \text{ if }S_{1} \subset Q \\
S_{1},\,  & \text{ otherwise}
\end{cases}
$$
Now if ${} S_{1} \in I(E) {}$, and ${} \varphi:E \to{}I(E)(S_{1}) {}$, then define
$$
R_{x}(\varphi)= S\left(  \bigcup \im \varphi \right)
$$
Then there exists some unique ${} F:E\to{}I(E) {}$ such that ${} F(x)=R_{x}(F \mid E(x)) {}$. Now ${} F(x) \subseteq Q {}$ for all ${} x \in Q {}$. Now suppose that ${} F(x) \subset Q {}$ for all ${} x \in E {}$. Then we have, for each element of $E$, an ideal strictly containing all previous ideals