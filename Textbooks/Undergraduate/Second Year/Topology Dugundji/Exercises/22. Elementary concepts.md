...
d)
Let $A$ be a closed (or open) set. Then ${} \Fr(A)=\conj{A} \cap \conj{\mathscr{C}A} {}$, and so, since ${} \conj{\Fr(A)} \subseteq \Fr(A) {}$, then ${} \Fr(A) {}$ is closed, and so
$$
\begin{align}
 \Int(\conj{\Fr(A)})=\Int(\Fr(A))  
 & =\mathscr{C} \conj{\mathscr{C}(\conj{A} \cap  \conj{\mathscr{C} A})}  \\
& =\mathscr{C} \conj{\mathscr{C}\conj{A} \cup \mathscr{C} \conj{\mathscr{C}A}} \\
 ...
 \end{align}
$$
Now if $A$ is closed, then ${} \conj{A}=A {}$, and let ${} x \in \Int(\Fr(A)) {}$. Then ${} x \in \Int(A) {}$, and ${} x \in \Int(\conj{\mathscr{C}A}). {}$ 