...
(${} b\Rightarrow a {}$). Let ${} B \subseteq Y {}$. Then let ${} A=f^{-1}(B) {}$, and so ${} f(A') \subseteq \conj{f(A)}=\conj{f(f^{-1}(B))}=\conj{f(X) \cap  B}\subseteq \conj{B} {}$ and ${} f^{-1}(B)' \subseteq f^{-1}(\conj{B}) {}$